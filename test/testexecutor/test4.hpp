...
// Context
struct MyContext {
  MyContext(uint32_t threads_num) : fiber_executor_(threads_num) {
    fiber_executor_.Start();
  }
  ...
  ytlib::FiberExecutor fiber_executor_;
};

template <typename Receiver>
inline void MyOperationState<Receiver>::start() noexcept {
  try {
    ctx_->fiber_executor_.Post([this] {
      try {
...
void Test4() {
  DBG_PRINT("[run in thread %llu]Test4 main.", ytlib::GetThreadId());
