 ...
 * @return std::size_t 字符串ps首次出现在ss中的位置，如果未出现则返回ss.length()
 */
inline std::size_t KMP(const std::string& ss, const std::string& ps) {
  std::size_t sslen = ss.length();
  std::size_t pslen = ps.length();
 ...
 * @param[in] s1 字符串1
 * @param[in] s2 字符串2
 * @return std::size_t 字符串1和字符串2的差异度
 */
inline std::size_t StrDif(const std::string& s1, const std::string& s2) {
  ...
  std::vector<std::size_t> v1(s2len + 1), v2(s2len + 1);
  std::size_t *M1 = v1.data(), *M2 = v2.data(), *tmpM;

  for (std::size_t ii = 0; ii <= s2len; ++ii) {
    M1[ii] = ii * c1;
  }
  ...
  for (std::size_t ii = 1; ii <= s1len; ++ii) {
    for (std::size_t jj = 0; jj <= s2len; ++jj) {
      if (jj == 0)
        ...
        std::size_t val1 = ((s1[ii - 1] == s2[jj - 1]) ? 0 : c2) + M1[jj - 1];
 ...
 * @param[in] s 字符串
 * @return std::pair<std::size_t, std::size_t> 最长不重复子串出现的位置和长度
 */
