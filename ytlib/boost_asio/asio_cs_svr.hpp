...
class AsioCsServer : public std::enable_shared_from_this<AsioCsServer> {
 public:
  using MsgHandleFunc = std::function<void(boost::asio::ip::tcp::endpoint, std::shared_ptr<boost::asio::streambuf>)>;

  struct Cfg {
    uint16_t port = 57634;  // 监听的端口
    ...
    static Cfg Verify(const Cfg& verify_cfg) {
      ...
      if (cfg.port > 65535 || cfg.port < 1000) cfg.port = 57634;
  ...
  AsioCsServer(std::shared_ptr<boost::asio::io_context> io_ptr, const AsioCsServer::Cfg& cfg)
  ...
  void SendMsg(const boost::asio::ip::tcp::endpoint& ep, std::shared_ptr<boost::asio::streambuf> msg_buf_ptr) {
    auto self = shared_from_this();
    boost::asio::dispatch(
        mgr_strand_,
        [this, self, ep, msg_buf_ptr]() {
          ASIO_DEBUG_HANDLE(cs_svr_send_msg_to_cli_co);
        ...
        [this, self]() -> boost::asio::awaitable<void> {
          ...
          while (run_flag_) {
            try {
              ...
              auto session_ptr = std::make_shared<AsioCsServer::Session>(boost::asio::make_strand(*io_ptr_), session_cfg_ptr_, io_ptr_, msg_handle_ptr_);
  ...
  class Session : public std::enable_shared_from_this<Session> {
   public:
    Session(boost::asio::strand<boost::asio::io_context::executor_type> session_strand,
            std::shared_ptr<const AsioCsServer::SessionCfg> session_cfg_ptr,
    ...
    void SendMsg(std::shared_ptr<boost::asio::streambuf> msg_buf_ptr) {
      ...
      boost::asio::dispatch(
          ...
          [this, self, msg_buf_ptr]() {
            ASIO_DEBUG_HANDLE(cs_svr_session_send_msg_to_cli_co);
    ...
    void Stop() {
      ...
      boost::asio::dispatch(
          session_strand_,
          [this, self]() {
            ASIO_DEBUG_HANDLE(cs_svr_session_stop_co);
