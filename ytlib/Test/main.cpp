...
void send_fun(myTcpNetAdapter* p,uint32_t id) {
	uint32_t count = 0;
	std::string s;
...
int32_t main(int32_t argc, char** argv) {
	...
	if (true) {
		...
		myTcpNetAdapter* c=new myTcpNetAdapter(3000, TcpEp(boost::asio::ip::tcp::v4(), 60003),
			std::bind(&handel_recv3, std::placeholders::_1));
		...
		for (int ii = 0; ii < 5; ++ii) {
			m_RunThreads.create_thread(std::bind(&send_fun, a, 2000));
			m_RunThreads.create_thread(std::bind(&send_fun, a, 3000));

			Sleep(100);
			m_RunThreads.create_thread(std::bind(&send_fun, b, 1000));
			m_RunThreads.create_thread(std::bind(&send_fun, b, 3000));

			Sleep(100);

			m_RunThreads.create_thread(std::bind(&send_fun, c, 1000));
